...
# ── 2b. Descargar Boost 1.60.0 headers (version exacta requerida por KenshiLib) ─
# Los headers usan tipos internos de boost::unordered que cambiaron en versiones
# posteriores. Siempre se usa 1.60.0.
Write-Step "Descargando Boost 1.60.0 headers..."
$boostDst = Join-Path $sdkRoot 'boost_1_60_0'

if (Test-Path (Join-Path $boostDst 'boost\version.hpp')) {
    Write-OK "Boost 1.60.0 ya presente en: $boostDst"
} else {
    $boostZip = Join-Path $tmpDir 'boost_1_60_0.zip'
    $boostUrl = 'https://archives.boost.io/release/1.60.0/source/boost_1_60_0.zip'
    Write-Host "  -> $boostUrl"
    ...
    Write-Host "  Extrayendo Boost 1.60.0..."
    ...
    Write-OK "Boost 1.60.0 en: $boostDst"
}
